...
7. In Windows Powershell, go to the folder where you installed openocd-xpack and then to .bin where the command is: <Path_to_openocd-xpack_Folder>\openocd-xpack\xpacks\.bin> 
8. Press STLink pogo pins to uMyo firmly and hold
9. Run the openocd command to upload firmware:  .\openocd -f interface/stlink.cfg -f target/nrf52.cfg -c init -c "reset halt" -c "flash write_image erase .<Path_to_uMyo_hex>/umyo/uMyo_v3_1.hex" -c "reset" -c exit
