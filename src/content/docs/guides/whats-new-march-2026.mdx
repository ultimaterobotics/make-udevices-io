...
uMyo streams data over Bluetooth LE to an Android phone, which forwards it to your PC over WiFi. This gives you real-time EMG, IMU, and magnetometer data in the workbench without a USB receiver base. Supports up to 6 simultaneous devices.
...
BLE advertisement mode still works. When uMyo is in BLE mode but no phone is connected, it broadcasts standard BLE advertisement packets containing spectrum and quaternion data in the manufacturer-specific field — the same format as before. ESP32 and other BLE scanners can receive these as usual.
